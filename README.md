...
```console
$ docker run --name some-mysql -e MYSQL_ROOT_PASSWORD=my-secret-pw -d nmoeini/mysql:tag
```

... where `some-mysql` is the name you want to assign to your container, `my-secret-pw` is the password to be set for the MySQL root user and `tag` is the tag specifying the image version you want.
...
```console
$ docker run -it --network some-network --rm nmoeini/mysql:tag mysql -h some-mysql -u example-user -p
```
...
```console
$ docker run -it --rm nmoeini/mysql:tag mysql -hsome.mysql.host -usome-mysql-user -p
```
...
```console
$ docker run --name some-mysql -v /my/custom:/etc/mysql/conf.d -e MYSQL_ROOT_PASSWORD=my-secret-pw -d nmoeini/mysql:tag
```
...
```console
$ docker run --name some-mysql -e MYSQL_ROOT_PASSWORD=my-secret-pw -d nmoeini/mysql:tag --character-set-server=utf8mb4 --collation-server=utf8mb4_unicode_ci
```

If you would like to see a complete list of available options, just run:

```console
$ docker run -it --rm nmoeini/mysql:tag --verbose --help
```
...
```console
$ docker run --name some-mysql -e MYSQL_ROOT_PASSWORD_FILE=/run/secrets/mysql-root -d nmoeini/mysql:tag
```
...
2.	Start your `mysql` container like this:
	...
	```console
	$ docker run --name some-mysql -v /my/own/datadir:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=my-secret-pw -d nmoeini/mysql:tag
	```
...
$ docker run -v "$PWD/data":/var/lib/mysql --user 1000:1000 --name some-mysql -e MYSQL_ROOT_PASSWORD=my-secret-pw -d nmoeini/mysql:tag
```
